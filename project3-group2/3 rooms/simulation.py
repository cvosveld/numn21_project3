...
A = None
gamma_1 = gamma_2 = gamma_N = gamma_H = gamma_WF = None
lu = None
ml = None
...
def solve_full_MPI():
    ...
    u1o,u2o,u3o = u1.copy(),u2.copy(),u3.copy()
    bc1o,bc2o = bc1.copy(),bc2.copy()
    ...
    for i in iterator:
        if rank==1:
            ...
            u2_new,bc1_new,bc2_new= solve_subdomain(subdomain2,bc1,bc2)
            ...
            u1o, u3o = u1.copy(), u3.copy()
            u2o = u2_new.copy()                              
            bc1o, bc2o = bc1_relaxed.copy(), bc2_relaxed.copy()
            u2 = u2_new
        
        elif rank==0:
            if i==0: subdomain1 = Subdomain(1, invdx, shape=(1,1), neumann_wall='E')
            comm.Recv(bc1,source = 1)
            u1_new,bc1_new = solve_subdomain(subdomain1,bc1,bc2)
            ...
            u3_new,bc2_new = solve_subdomain(subdomain3,bc1,bc2)
    ...
    if rank==1:
        ...
        ud[1*invdx:, 2*invdx-1:] = u3d
        return ud

def solve_subdomain(subdomain,bc1,bc2):
    global gamma_1, gamma_2, gamma_N, gamma_H, gamma_WF, A, ml
    ...
    if any(g is None for g in [gamma_1, gamma_2, gamma_N, gamma_H, gamma_WF]) or gamma_1.size != N:
        gamma_1, gamma_2, gamma_N, gamma_H, gamma_WF = calculate_gammas(subdomain)
    
    if subdomain.id==2:
        u_gamma_1 = bc1; u_gamma_2 = bc2; 
        b1 = np.zeros(N, dtype='d'); b1[gamma_1==1] = u_gamma_1
        b2 = np.zeros(N, dtype='d'); b2[gamma_2==1] = u_gamma_2
        b = -(b1 + b2 + gamma_N*u_N + gamma_H*u_H + gamma_WF*u_WF)/dx**2
